...
@ContextConfiguration(
		classes = {
				...
				NatsMessageAsyncProducingHandlerManualTest.ContextConfig.class
		})
public class NatsMessageAsyncProducingHandlerManualTest extends AbstractNatsIntegrationTestSupport {
	 ...
	 * NatsMessageAsyncProducingHandlerManualTest.ContextConfig#testRecurringPaymentInBoundFlow(NatsTemplate)}
	 * Bean context for outbound flow {@link
	 * NatsMessageAsyncProducingHandlerManualTest.ContextConfig#negativeFlow()}
	 *
	 * <p>Test scenario: Non-breakable message producing during unavailability of NATS server
	 * [Producer Internal queue capacity(10) = send messages more than this capacity (&gt; 100%)]
	 * Insure all messages has been delivered even if NATS Server was not available.
	 *
	 * <p>Result expected: Message should be endless re-sent until NATS server again up and running
	 * and delivered to consumer in any case without duplicates
	 *
	 ...
	 */
	@Test
	public void testAsyncProducerWithQueueCapacityOverFlow() throws Exception {
		resetStatistics();
		...
		// 3. trying to send the messages to not available server to see the behaviour, more than 10
		// messages ( more than full queue capacity)
		LOG.info("Trying to send to not available NATS server....");
		sendBunchMessages(5, 30);
