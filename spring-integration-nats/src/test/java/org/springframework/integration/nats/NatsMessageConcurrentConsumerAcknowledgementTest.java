...
@ContextConfiguration(
		classes = {
				...
				NatsMessageConcurrentConsumerAcknowledgementTest.ContextConfig.class
		})
public class NatsMessageConcurrentConsumerAcknowledgementTest
		extends AbstractNatsIntegrationTestSupport {

	public static final int MAX_REDELIVER = 3;

	private static final Log LOG = LogFactory.getLog(NatsMessageConcurrentConsumerAcknowledgementTest.class);
	...
	@Autowired
	private NatsMessageConcurrentConsumerAcknowledgementTest.ControlBusGateway controlBus;
	 ...
	 * <p>Bean context for outbound flow {@link ContextConfig#consumerInboundFlow()}
	 *
	 * <p>Test scenario: Message processing takes longer than the specified ACK timeout for consumer
	 *
	 * <p>ACK_WAIT for consumer is configured as 2 seconds in NATS server
	 *
	 * <p>But message processing takes 10 seconds, so this message will be redelivered Result
	 * expected: Message will be redelivered based on the ack_wait timeout and max redeliver count
	 *
	 * @throws IOException           covers various communication issues with the NATS server such as timeout or
	 *                               interruption
	 * @throws JetStreamApiException the request had an error related to the data
	...
	public void testMessageRedeliveryForAckTimeoutOnDelayedProcessing() throws Exception {
		...
		LOG.info("-----------------------------------------------");
		LOG.info("No of time messages redelivered with timestamp");
	...
	public static class ContextConfig {
		...
		public void processMessage(@Payload final Message<?> message)
			...
			LOG.info(
					"Message received in service Activator: "
							+ key
							+ " Subscription ID => "
							+ natsMessage.getSID());
